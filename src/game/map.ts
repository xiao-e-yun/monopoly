...
export class GameMap {
  constructor(rawMap: Tile[][], public spawnPointDirections: GameInputMove[]) {
    this.tiles = rawMap;
  ...
  spawnPoints(): [[number, number], GameInputMove][] {
    ...
    const binded = this.spawnPointDirections.map(
      (direction, index) =>
        [spawners[index], direction] as [[number, number], GameInputMove]
    );
  ...
  nextPosition(
    ...
    direction?: GameInputMove
  ): Map<GameInputMove, [number, number]> {
