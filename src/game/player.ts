...
export const DEFAULT_PLAYER_HEALTH = 5
export class Player {
  ...
  async trigger() {
    ...
    const callback = ({
      [Tile.Spawner]: async () => {
        console.log(`獲得 50 分`)
        state.messages.push(`第 ${this.id} 組 獲得 50 分`)
        this.score += 50

        const victoryScore = state.victoryScore
        if (victoryScore <= this.score) {
          console.log(`獲勝`)
          state.messages.push(`第 ${this.id} 組 獲勝`)
          state.winner = this
      ...
      [Tile.Task]: async () => {
        ...
        state.messages.push(`第 ${this.id} 組 獲得任務 ${task[0]}`)
      ...
      [Tile.Opportunity]: async () => {
        ...
        state.messages.push(`第 ${this.id} 組 獲得機會 ${opportunity[0]}`)
      ...
      [Tile.Destiny]: async () => {
        ...
        state.messages.push(`第 ${this.id} 組 獲得命運 ${destiny[0]}`)
      ...
      [Tile.Punishment]: async () => {
        ...
        state.messages.push(`第 ${this.id} 組 獲得懲罰 ${punishment[0]}`)
      ...
      [Tile.Teleport]: async () => {
        ...
        state.messages.push(`第 ${this.id} 組 傳送至 ${teleport}`)
      ...
      [Tile.Prison]: async () => {
        console.log(`被關 1 回合`)
        state.messages.push(`第 ${this.id} 組 被關 1 回合`)
        this.dizziness++
      },

      [Tile.Hospital]: async () => {
        console.log(`被治療 1 回合`)
        state.messages.push(`第 ${this.id} 組 被治療 1 回合`)
